...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\release-local.ps1 -Version 0.2.6
```

Windows 需要签名时：

```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\release-local.ps1 -Version 0.2.6 -RequireSignature
```
...
```bash
./scripts/check-release-env.sh
./scripts/release-local.sh 0.2.6
```
